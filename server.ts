...
async function startServer() {
  ...
  app.post('/api/auth/login', async (req, res) => {
    try {
      ...
      // Log Auth Event & Send Throttled Notification
      const authEvent = db.logAuthEvent({
        userId: user.id,
        eventType: 'LOGIN',
        provider: 'EMAIL',
  ...
  app.post('/api/auth/oauth', async (req, res) => {
    try {
      ...
      const profilePhotoUrl = req.body?.profilePhotoUrl || (provider === 'google'
      ...
      // Graceful fallback: return valid session so client is NEVER blocked
      const rawProvider = String(req.body?.provider || 'google').toLowerCase();
